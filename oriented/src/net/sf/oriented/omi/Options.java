 ...
 * See {@link FactoryFactory#FactoryFactory(Options)}.
 * @author jeremy
 *
 */
public class Options {
	...
	private String implementation = Options.class.getPackage().getName()+".impl.set.bits32.";
	...
	public <T> JavaSet<T> javaSetFor(Class<T> cl) {
		try {
		if (javaSet==null) {
				javaSet = (Class<JavaSet<?>>) Class.forName(implementation+"JavaHashSet");
			} 
	...
	public <T> Constructor<?> constructorFor(Class<T> fc) {
		...
		try {
			Class<T> impl = (Class<T>) Class.forName(implementation+name+"Impl");
			Constructor<?> c[] = impl.getConstructors();
			if (c.length != 1)
				throw new RuntimeException("internal problem: more than one constructor found for "+name+"Impl");
			return  c[0];
		} catch (ClassNotFoundException e) {
			return null;
		}
...
//	}
	...
	/**
	 * All the {@link Label}'s used consist of a single character (unicode code point).
	 * Use short forms for everything.
	 *
	 */
	...
	/**
	 * Some of the {@link Label}'s used consist of more than one character.
	 * Use longer forms for everything.
	 *
	 */
	...
	/**
	 * The only {@link Label}'s used are those passed in
	 * a call to {@link #setUniverse(Label[])}.
	 * Any other {@link Label} will cause an IllegalArgumentException.
	 *
	 */
