...
final public class SetOfSignedSetImpl
	...
	SetOfSignedSetImpl bb = remakex(b);
	if (members.length != bb.members.length) return false;
	if (hashCode != MARKER && bb.hashCode != MARKER
		&& hashCode != bb.hashCode) return false;
	for (int i = 0; i < members.length; i++)
	    if (members[i] != bb.members[i]) return false;
	return true;
	...
	if (hashCode == MARKER) {
	    ...
	    hashCode = 0;
	    for (int i = 0; i < members.length; i++) {
		hashCode += lf.hashCode(plus(members[i])) + 35
			* lf.hashCode(SignedSetFactory.minus(members[i]));
	    }
	}
	return hashCode;
    }
