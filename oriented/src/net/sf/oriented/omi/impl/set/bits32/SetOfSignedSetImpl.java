 ...
 * 
 */
final public class SetOfSignedSetImpl extends SetImpl
< SignedSetInternal,
...
public SetOfSignedSetFactory factory() {
	...
	private UnsignedSetInternal computeSupport() {
		...
		for (int i=0;i<members.length;i++) {
			s |= plus(members[i])|minus(members[i]);
		}
		return make(s);
	}
...
//			switch (s.sign(l)) {
//			case -1:
//				r.add(new SignedSetImpl(s.plus(),s.minus().minus(l),factory().itemFactory()));
//				break;
//			case 0:
//				r.add(s);
//				break;
//			case 1:
//				r.add(new SignedSetImpl(s.plus().minus(l),s.minus(),factory().itemFactory()));
//				break;
//			}
//		}
...
//	}
	...
	public boolean sameSetAs(SetOfSignedSet b) {
		...
		if (members.length!=bb.members.length)
		   return false;
		if (hashCode!=MARKER && bb.hashCode!=MARKER && hashCode != bb.hashCode)
			return false;
		for (int i=0;i<members.length;i++)
	...
	public int hashCode() {
		if (hashCode==MARKER) {
			...
			for (int i=0;i<members.length;i++) {
				hashCode += 
					  lf.hashCode(plus(members[i]))+ 
					  35 * lf.hashCode(SignedSetFactory.minus(members[i]));
			}
		}
		return hashCode;
	}
	...
	public SetOfSignedSetInternal withSupport(UnsignedSetInternal u) {
		...
		for (int i=0;i<members.length;i++) {
			if (want == (plus(members[i])|minus(members[i])) )
				m[ok++] = members[i];
		}
		return make(ok,m);
	}
	...
	public SetOfSignedSetInternal conformingWith(SignedSetInternal x) {
		...
		for (int i=0;i<members.length;i++) {
			if (conforming(plus(members[i]),minus(members[i]),p,m) )
				mems[ok++] = members[i];
		}
		return make(ok,mems);
	}
