...
public class OMChirotopeFactory 
extends MoreAbsFactory<OMChirotope,Pair> 
...
implements ChirotopeFactory {
	...
	@Override
	public OMChirotope fromChirotope(Chirotope chi) {
		final int n = chi.n();
		return fromChirotope(new AbstractCollection<Label>() {
			@Override
	...
	@Override
	protected OMChirotope construct(Collection<? extends Label> ground, final Pair defn) {
		final int n = ground.size();
		return 
		  fromChirotope(ground,new Chirotope(){
			@Override
			public int chi(int... i) {
				...
				case '0':
					return 0;
						default:
							throw new IllegalArgumentException("Syntax error in chirotope defn string: "+ch);
				}
			}
			@Override
			public int n() {
	...
	@Override
	public OMChirotope fromChirotope(Collection<? extends Label> e, Chirotope chi) {
