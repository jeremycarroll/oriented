...
public class ChirotopeImpl extends AbsOM implements OMChirotope {
	...
	for (int q = 1; q < r; q++)
	    if (i[p + q] > i[p])
		i[p + q] -= (i[p] + 1);
	    else throw new IllegalArgumentException("sequence not monotonic");
	int rslt = 0;
	int nextN = n - 1 - i[p];
	while (i[p] > 0) {
	...
	if (independent.size() == rank()) {
	    if (bases.contains(independent)) return independent;
	} else {
	    Iterator<UnsignedSetInternal> it = bases.iterator();
	    while (it.hasNext()) {
		UnsignedSetInternal base = it.next();
		if (base.isSupersetOf(independent)) return base;
	    }
	}
