...
//		   = vectors.withSupport(vvv);
	...
	public SetOfUnsignedSetInternal circuits() {
		JavaSet<UnsignedSetInternal> dependent[] = 
			new JavaSet[ground().length+1];
		for (int i=0;i<dependent.length;i++)
			dependent[i] =
			all.unsignedSets().emptyCollectionOf();
		Iterator<SignedSetInternal> it = iterator();
		while (it.hasNext()) {
			UnsignedSetInternal dep = it.next().support();
            dependent[dep.size()].add(dep);
		}
		JavaSet<UnsignedSetInternal> circuits = all.unsignedSets().emptyCollectionOf();
		...
		return ((SetOfUnsignedSetFactory)ffactory().setsOfUnsignedSet()).fromBackingCollection(circuits);
	}

	private boolean addIfNoSubsets(JavaSet<UnsignedSetInternal> circuits, JavaSet<UnsignedSetInternal> v,
			long present) {
