...
public class OMAll implements OMInternal {
	...
	case COCIRCUITS:
	    ...
	    if (has(COVECTORS)) return new Circuits(getChirotope());

	    if (has(MAXVECTORS)) return new Circuits(getVectors());
	    return new Circuits(getChirotope());
	case VECTORS:
	case COVECTORS:
	    ...
	    if (has(MAXVECTORS)) return new Vectors(getMaxVectors());
	    return new Vectors(getCircuits());
	case MAXVECTORS:
	case TOPES:
	...
	for (OMInternal f : forms)
	    if (f != null && !f.verify()) return false;
    ...
    @Override
    public int rank() {
	if (has(CHIROTOPE)) return get(CHIROTOPE).rank();
	...
	Label g[] = o.ground();
	if (g.length != ground.length) return false;
	outer: for (int i = 0; i < g.length; i++) {
	    for (int j = 0; j < g.length; j++)
		if (ground[j].equals(g[i])) continue outer;
	    return false;
	}
	return true;
    }
	...
	if (o == null || (!(o instanceof OM))) return false;
	if (o == this) {
	    return true;
	}
	...
	int i = hasSameForm(a);
	if (i != -1) return forms[i].equals(a.forms[i]);
	i = dual.hasSameForm(a.dual);
	if (i != -1) return dual.forms[i].equals(a.dual.forms[i]);
	return equalsByCircuits(a);
    }

    private int hasSameForm(OMAll a) {
	for (int i = 0; i < forms.length; i++)
	    if (forms[i] != null && a.forms[i] != null) return i;
	return -1;
    }

    boolean equalsByCircuits(OM o) {
	if (!sameGroundAs(o)) return false;
