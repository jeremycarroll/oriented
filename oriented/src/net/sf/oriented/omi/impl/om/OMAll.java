...
public class OMAll implements OMInternal {
	...
	/**
	 * allows distinguishing the two parts of a dual pair.
	 * Notice these twins, the older one starts coming out
	 * before the younger, but the younger one comes out first!
	 */
   ...
   private void set(Cryptomorphisms which, int ix, OMInternal definedOM) {
    ...
    private OMInternal get(int ix, Cryptomorphisms which) {
		...
		case VECTORS:
		case COVECTORS:
			if (has(CIRCUITS) || has(CHIROTOPE) || has(DUALCHIROTOPE) || has(COCIRCUITS)
					|| has(COVECTORS)) {
	...
	public MatroidInternal getMatroid() {
		if (!matroid.isSet()) {
			if (has(CHIROTOPE) ) {
				...
				throw new IllegalStateException("Oriented matroid without a representation.");
			}
		}
		return matroid;
	}
