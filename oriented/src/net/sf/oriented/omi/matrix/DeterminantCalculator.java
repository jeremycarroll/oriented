...
class DeterminantCalculator {
	...
	rank = h;
	 FieldMatrix<PerisicFieldElement> m = 
		 new Array2DRowFieldMatrix<PerisicFieldElement>(PerisicField.QPolynomials,h,h);
	...
	RingElt quotient = new FieldLUDecomposition<PerisicFieldElement>(m).getDeterminant().getDelegate();

	assert(  ((QuotientField)quotient.getRing()).denominator(quotient).equals(PerisicField.U.one()) );
    ...
    public PerisicFieldElement compute( FieldMatrix<PerisicFieldElement> delegate, int[] indices) {
	final String vars[] = new String[rank*rank];
	final RingElt values[] = new RingElt[rank*rank];
	for (int i=0;i<rank;i++) {
	    final int ii = i;
	delegate.walkInOptimizedOrder(new FieldMatrixPreservingVisitor<PerisicFieldElement>(){
