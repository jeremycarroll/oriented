...
final class NaiveMinimalSubsets implements MinimalSubsets {
    ...
    int max = 0;
    private static final Comparator<SetAndComplement> BY_CARDINALITY = new Comparator<SetAndComplement>() {
    ...
    public List<BitSet> minimal(Collection<BitSet> full) {
        int i;
        SetAndComplement[] r = prepareData(full);

        int sz = r.length;
        int bad = 0;
        Arrays.sort(r, BY_CARDINALITY);
        for (i = 0; i < sz - 1; i++) {
        ...
        return gatherResults(r, bad);
    }

    private SetAndComplement[] prepareData(Collection<BitSet> full) {
    ...
    private List<BitSet> gatherResults(SetAndComplement[] r, int bad) {
        int i;
        int sz = r.length;
